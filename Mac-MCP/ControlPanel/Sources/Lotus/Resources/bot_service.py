...
=============================================
Bundled inside Lotus.app/Contents/Resources/.
Config is read from ~/Library/Application Support/Lotus/config.json
(written by the Swift control panel) — never from inside the bundle.
"""
...
# Config lives in the writable app-data dir, not inside the (possibly read-only) bundle.
...
def load_config() -> dict | None:
    if CONFIG_FILE.exists():
        ...
        _log.error("config.json not found at %s — run the Lotus app first", CONFIG_FILE)
    return None
...
def run_service() -> None:
    ...
    if not token:
        _log.error("No token in config — run the Lotus app first.")
        sys.exit(1)
