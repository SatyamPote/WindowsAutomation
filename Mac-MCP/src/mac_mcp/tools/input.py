...
def register(mcp, *, get_desktop, get_analytics):
    ...
    def click_tool(
        ...
        return f"{num_clicks.get(clicks, str(clicks))} {button} clicked at ({x},{y})."

    @mcp.tool(
        name="Type",
        description=(
            "Types text at specified coordinates [x, y] or passing a UI element's label/id. "
    ...
    @mcp.tool(
        name="Scroll",
        description=(
            "Scrolls at coordinates [x, y], a UI element's label/id, or current mouse position if loc=None. "
            "Type: vertical (default) or horizontal. Direction: up/down for vertical, left/right for horizontal. "
            "wheel_times controls amount (1 wheel ≈ 3-5 lines). Use for navigating long content, lists, and web pages."
        ),
        annotations=ToolAnnotations(
            title="Scroll",
            readOnlyHint=False,
    ...
    def scroll_tool(
        ...
        return f"Scrolled {type} {direction} by {wheel_times} wheel times{pos}."
    ...
    @mcp.tool(
        ...
        description=(
            ...
            'Examples: "cmd+c" (copy), "cmd+v" (paste), "cmd+tab" (switch apps), '
            '"cmd+space" (Spotlight), "cmd+shift+3" (screenshot), "ctrl+shift+esc" (force quit). '
            'Use cmd for the Command key, option/alt for Option, ctrl for Control. '
    ...
    def shortcut_tool(shortcut: str, ctx: Context = None) -> str:
    ...
    @mcp.tool(
        name="Wait",
        description=(
            "Pauses execution for specified duration in seconds. Use when waiting for: "
            "applications to launch/load, UI animations to complete, page content to render, "
            "dialogs to appear, or between rapid actions. Helps ensure UI is ready before next interaction."
        ),
        annotations=ToolAnnotations(
            title="Wait",
            readOnlyHint=True,
