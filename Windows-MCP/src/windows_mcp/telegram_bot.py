...
def _load_version() -> str:
    """Single source of truth: version.json in the install root."""
    import json as _json, sys as _sys
    candidates = []
    if getattr(_sys, "frozen", False):
        candidates.append(os.path.join(os.path.dirname(_sys.executable), "version.json"))
        meipass = getattr(_sys, "_MEIPASS", None)
        if meipass:
            candidates.append(os.path.join(meipass, "version.json"))
    here = os.path.dirname(os.path.abspath(__file__))
    candidates.append(os.path.normpath(os.path.join(here, "..", "..", "version.json")))
    for p in candidates:
        if os.path.isfile(p):
            try:
                with open(p, "r", encoding="utf-8") as _f:
                    return _json.load(_f).get("version", "3.0.0")
            except Exception:
                pass
    return "3.0.0"
...
# Managers (playlists are owned by music_player — single source of truth)
...
def clean_input(text: str) -> str:
    ...
    t = text.strip().lower()
    # Remove fillers — but NEVER strip command tokens like "research" or "on"
    # (they are routed on explicitly in parse_and_execute).
    t = re.sub(r'\b(about)\b', '', t)
...
# Verbs that should NEVER be intercepted by the file-search branch.
# They go to the launcher / scrape / power tools first.
_APP_LAUNCH_VERBS = {"open", "launch", "start", "run"}
_FILE_VERBS = {"send", "find", "show file", "locate"}

# Words that look like real commands (used to suppress AI fallback for
# clear command typos so the user gets a real error instead of LLM noise).
_COMMAND_WORDS = {
    "open", "close", "launch", "start", "run", "kill",
    "play", "pause", "resume", "stop", "next", "prev", "volume",
    "find", "send", "locate", "ls", "cd", "tree", "delete", "compress",
    "research", "download", "say", "voice", "screenshot", "record",
    "lock", "sleep", "shutdown", "restart", "dashboard", "storage",
    "create", "add", "remove", "list", "playlist",
}


def _is_app_query(query: str) -> bool:
    """Heuristic: does this look like an application name (not a file)?"""
    q = query.lower().strip()
    if not q:
        return False
    if q in APP_MAPPING:
        return True
    # No file extension AND short → probably an app
    if "." not in q and len(q.split()) <= 3:
        return True
    return False


def _search_files(query: str) -> str | None:
    """Search the storage tree + Downloads for the first matching file."""
    search_dirs = [
        STORAGE_DIR, STORAGE_RESEARCH,
        os.path.join(os.path.expanduser("~"), "Downloads"),
    ]
    q = query.lower()
...
async def parse_and_execute(text: str, update: Update, context: ContextTypes.DEFAULT_TYPE) -> dict:
    """STRICT PRIORITY ROUTER: System > Apps > Files > Music > Research > Voice > AI fallback.

    Each branch returns explicitly. AI fallback only fires when the input
    contains zero command words AND zero of the recognized verbs above.
    """
    raw_text = text
    t = clean_input(text)
    first_word = t.split()[0] if t else ""
    rest = t[len(first_word):].strip() if first_word else ""

    # ── 1. SYSTEM COMMANDS ─────────────────────────────────────────────
    sys_cmds = {"shutdown", "restart", "lock", "sleep", "storage status",
                "clear storage", "show logs", "clear logs", "dashboard"}
    if t in sys_cmds:
        if t == "storage status":
        ...
        if t == "clear storage":
            return {"success": True, "message": storage_manager.clear_storage()}
        if t == "show logs":
            return {"success": True, "message": f"📜 *Logs:*\n`{activity_logger.get_logs(10)}`"}
        if t == "clear logs":
            return {"success": True, "message": activity_logger.clear_logs()}
        if t == "dashboard":
            stats = load_stats()
    ...
    # ── 2. APP LAUNCH (open chrome, launch vscode, …) ──────────────────
    # IMPORTANT: this must come before file search so `open chrome` doesn't
    # try to find a file called "chrome" on disk. If launch fails or the
    # query doesn't look like a known app, fall through to the file branch
    # so `open report.pdf` still works.
    if first_word in _APP_LAUNCH_VERBS and rest:
        if _is_app_query(rest):
            target = APP_MAPPING.get(rest, rest)
            try:
                exe = find_executable(target, APP_CACHE) or target
                if launch_app(exe):
                    track_command("openapp")
                    return {"success": True, "message": f"🚀 Launched: `{rest}`"}
            except Exception as e:
                log_error(text, e)
            # Launch failed → fall through to file search below

    # ── 3. FILE COMMANDS ───────────────────────────────────────────────
    if first_word in _FILE_VERBS or (first_word in _APP_LAUNCH_VERBS and rest):
        if not rest:
            return {"success": False, "message": f"❓ {first_word} what? Usage: `{first_word} <name>`"}
        path = _search_files(rest)
        if path:
            if first_word in _APP_LAUNCH_VERBS:
                try:
                    os.startfile(path)
            ...
            return {"success": True, "message": f"📍 Found: `{path}`"}

        # Broad fuzzy find
        if first_word == "find":
            matches = find_file(rest)
            if matches:
                context.user_data["last_files"] = matches[:5]
                resp = "🔍 *Found matches:*\n" + "\n".join(
                    [f"{i+1}. `{os.path.basename(m)}`" for i, m in enumerate(matches[:5])]
                )
                return {"success": True, "message": resp}
            return {"success": False, "message": f"🔍 No files match `{rest}`."}

        # Explicit miss for open/send — do NOT fall through to AI
        return {"success": False, "message": f"❌ `{rest}` not found in storage or Downloads."}
    ...
    if first_word == "play" and not play_pl:
        query = rest
        if not query:
            return {"success": False, "message": "❓ Play what? Try `play lo-fi`."}
    ...
    if t == "next":
        return music_player.next_song()
    if t == "now playing":
        return music_player.now_playing()
    if t == "list playlists":
        return music_player.list_playlists()

    if vol_cmd:
        return music_player.set_volume(vol_cmd.group(1))
    if create_pl:
        return music_player.create_playlist(create_pl.group(1).strip())
    if delete_pl:
        return music_player.delete_playlist(delete_pl.group(1).strip())
    if add_pl:
        return music_player.add_to_playlist(add_pl.group(1), add_pl.group(2))
    if play_pl:
        return music_player.play_playlist(play_pl.group(1).strip())

    # ── 5. RESEARCH / DOWNLOAD ─────────────────────────────────────────
    if first_word == "research":
        topic = rest
        if topic:
            return {"success": True, "message": f"__RESEARCH__:{topic}"}
        return {"success": False, "message": "❓ Research what? Try `research black holes`."}
    ...
    if t == "voice off":
        return {"success": True, "message": voice_system.toggle_voice(False)}
    if t == "stop voice":
    ...
    if first_word == "say":
        if rest:
            voice_system.speak(rest)
            return {"success": True, "message": f"🗣️ Speaking: {rest}"}
    ...
    # ── 7. AI CHAT FALLBACK (only for genuine chat) ────────────────────
    # Suppress the AI for messages that *look* like commands but didn't
    # match — return an explicit error so the user can fix the typo.
    if first_word in _COMMAND_WORDS:
        return {"success": False, "message": f"❓ Unrecognized `{first_word}` command. Try `/help`."}
...
async def cmd_version(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        from windows_mcp.updater import check_for_updates
        result = await asyncio.to_thread(check_for_updates)
        await update.message.reply_text(
            result.get("message", f"🌸 Lotus v{VERSION}"),
            parse_mode="Markdown",
            disable_web_page_preview=True,
        )
    except Exception as e:
        log_error("/version", e)
        await update.message.reply_text(f"🌸 *Lotus v{VERSION}*\n_Update check unavailable._",
                                        parse_mode="Markdown")
...
def _frame(icon: str, body: str) -> str:
    """Single source of truth for response formatting. Strips redundant
    frames if the inner body already contains its own divider line."""
    body = (body or "").strip()
    if "━━━━" in body:
        return f"{icon} {body}" if icon else body
    return f"{_FRAME_TOP}{icon} {body}{_FRAME_BTM}" if icon else f"{_FRAME_TOP}{body}{_FRAME_BTM}"


def _is_duplicate(user_id: int, text: str) -> bool:
    """Drop the same command from the same user within _DEDUP_WINDOW_SEC."""
    key = f"{user_id}:{text.strip().lower()}"
    now = time.time()
    last = RECENT_COMMANDS.get(key)
    RECENT_COMMANDS[key] = now
    # Periodic prune
    if len(RECENT_COMMANDS) > 200:
        cutoff = now - 60
        for k in list(RECENT_COMMANDS):
            if RECENT_COMMANDS[k] < cutoff:
                RECENT_COMMANDS.pop(k, None)
    return last is not None and (now - last) < _DEDUP_WINDOW_SEC
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Drop accidental double-sends / Telegram retries
    if _is_duplicate(update.effective_user.id, text):
        logger.info("Dropped duplicate command: %s", text)
        return

    # Aliases (kept for the few code paths that still reference them inside this fn)
    FRAME_TOP = _FRAME_TOP
    FRAME_BTM = _FRAME_BTM
