  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // Only auto-scroll if the memo doesn't have focus (allows user to select text)
        if not LogMemo.Focused then
...
begin
  if FileExists(LogFilePath) then
    ShellExec('open', 'notepad.exe', LogFilePath, '', SW_SHOWNORMAL, ewNoWait, ErrorCode)
  else
    MsgBox('Log file not found yet. Please wait for installation to begin.', mbInformation, MB_OK);
end;
...
begin
  ...
  LogButton := TNewButton.Create(CustomInstallPage);
  LogButton.Parent := CustomInstallPage.Surface;
  LogButton.Left := CustomInstallPage.SurfaceWidth - 120;
  LogButton.Top := 0;
  LogButton.Width := 120;
  LogButton.Height := 22;
  LogButton.Caption := 'Open Detailed Log';
...
begin
  ...
  try
    ...
    if ModelName = 'skip' then
      ...
      ResultModel := RunHidden(ScriptsDir + '\download_model.ps1 -ModelName ' + ModelName, '85% -> Downloading AI model...', 85);
    ...
    EnvContent[4] := '  "created_at": "Installer Setup"';
    EnvContent[5] := '}';
    ...
    if ResultModel = -1 then
      Summary := Summary + '⏭ AI Model download (skipped by user)' + #13#10
    else begin
      Summary := Summary + StatusIcon(ResultModel) + ' AI Model (' + ModelName + ')';
      if ResultModel <> 0 then begin Summary := Summary + ' (FAILED)'; HasFailure := True; end;
      Summary := Summary + #13#10;
    end;
